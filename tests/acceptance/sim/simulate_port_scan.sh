#!/bin/bash
# ============================================================
# Simulate: Nmap-style Port Scan (triggers Wazuh rule 100211)
# ============================================================
# Probes the deterministic TCP ports prepared by setup_prereqs.sh.
# Those ports are explicitly denied+logged by UFW on the target so the
# test does not depend on whether arbitrary ports happen to be open.
#
# Uses SYN scan when run as root; otherwise uses TCP connect scan. The
# old script always used -sS and then hid nmap failures with `|| true`,
# which could print [OK] without sending a real scan.
#
# USAGE:
...
echo "==> Simulating port scan against $TARGET_HOST (${#PORTS[@]} explicitly logged test ports)"
...
echo "FAST rule 100210 should appear for blocked probes; correlation rule 100211 should fire after 8+ logged probes within 60s."
