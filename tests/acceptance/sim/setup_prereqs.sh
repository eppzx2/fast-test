...
#
#   1. OpenSSH server -> required for the brute-force simulation to
#      have anything to connect to (without it, sshd never runs, so
#      no auth events - successful or failed - are ever generated)
#   2. Port scan detection  -> enables UFW + connection logging
#   3. LOLBin detection     -> installs auditd + an execve watch rule
#
...
echo "==> [1/3] Installing and enabling OpenSSH server (target for brute-force sim)..."
...
echo ""
echo "==> [2/3] Enabling UFW with connection logging (for port-scan detection)..."
...
echo ""
echo "==> [3/3] Installing and configuring auditd (for LOLBin detection)..."
