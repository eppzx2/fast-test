...
### Troubleshooting: Docker Daemon Permission Denied

If `deploy.sh` (or `./bin/fast up`) fails partway through with an
error like:
...
this means your user isn't in the `docker` group yet (common on a
freshly installed VM/distro, e.g. Kali), even though the `docker
--version` check earlier succeeded. Fix it, then retry:

```bash
sudo usermod -aG docker $USER
newgrp docker
docker ps    # should now run without "permission denied"
```

If `newgrp docker` doesn't fully take effect, log out and log back in
(or start a new SSH session) instead — that always refreshes group
membership.

Because the failed run may have left a partially-cloned Wazuh stack
behind, clean it up before retrying:

```bash
rm -rf wazuh-docker
./bin/fast up      # or ./deploy.sh
```
